...
public class Game
{
    ...
    public string Mantra => $"game.requirement.round.{Round}";
    ...
    internal bool DropCardAtPlayerArea(Player player, string cardName, int cardIndex, Player playerToDrop, string dropAreaName)
    {
        ...
        if (!playerToDrop.DroppedCards[listIndex].Any(c => c.DisplayName == "empty") &&
            playerToDrop.DroppedCards[listIndex].Any())
        {
    ...
    private bool CheckIfDeckIsRun(List<Card> run)
    {
        ...
        var deck = run.GroupBy(suit => suit.Suit).Select(element => element.ToList()).ToList();
        ...
        deck = run.GroupBy(suit => suit.Suit).Where(c => c.Count() >= 4).Select(element => element.ToList()).ToList();
        if (deck.Count != 1)
        {
            return false;
        }
